...
mem_t *init_mem(int len)
{
    ...
    len = ((len+BLK_SIZE-1)/BLK_SIZE)*BLK_SIZE;//BLK_SIZE=32
    m->len = len;
    ...
    switch (icode) {
      ...
      {        
              ...
              if (!get_byte_val(sim->m, next_pc, &regs)) {
                   err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                   return STAT_ADR;}
              regid_t regA=GET_REGA(regs);
              regid_t regB=GET_REGB(regs);
              long_t valA=get_reg_val(sim->r,regA);
              if (cond_doit(sim->cc,ifun)==TRUE) set_reg_val(sim->r,regB,valA);
              next_pc++;
              sim->pc=next_pc;
              break;
       }
      ...
      {  
              ...
              next_pc++;
              long_t num=0;
              if (!get_long_val(sim->m, next_pc, &num)) {
                   err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                   return STAT_ADR;}
              set_reg_val(sim->r,regB,num);
              next_pc+=8;
              sim->pc=next_pc;
              break;
      }
      case I_RMMOVQ: /* 4:0 regA:regB imm */
      {
              ...
              next_pc++;
              long_t offset=0;
              if (!get_long_val(sim->m,next_pc,&offset)){
                   err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                   return STAT_ADR;}
              long_t valA=get_reg_val(sim->r,regA);
              long_t valB=get_reg_val(sim->r,regB);
              set_long_val(sim->m,valB+offset,valA);
              next_pc+=8;
              sim->pc=next_pc;
              break;
        }
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        {
              ...
              long_t valM=0;
              long_t valB=get_reg_val(sim->r,regB);
              if (!get_long_val(sim->m, valB+offset, &valM)) {
                   err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,valB+offset);
                   return STAT_ADR;}
     ...
     {
              long_t dest=0;
              if (!get_long_val(sim->m, next_pc, &dest)) {
                   err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                   return STAT_ADR;}
              next_pc+=8;
              if (cond_doit(sim->cc,ifun)) next_pc=dest;
              sim->pc=next_pc;
              break;
     }
      ...
      case I_CALL: /* 8:x imm */
      {
              ...
              rsp=get_reg_val(sim->r,4);
              set_long_val(sim->m,rsp,next_pc);
              long_t check=0;
              if (!get_long_val(sim->m, rsp, &check)) {
      ...
      case I_RET: /* 9:0 */
      {
              ...
              set_reg_val(sim->r,4,rsp+8);
              long_t retaddr=0;
              if (!get_long_val(sim->m, rsp, &retaddr)) {
                   err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,rsp);
                   return STAT_ADR;}
              sim->pc=retaddr;
              break;
      ...
      case I_POPQ: /* B:0 regA:F */
        {
              ...
              long_t rsp=get_reg_val(sim->r,4);
              long_t res=0;
              if (!get_long_val(sim->m, rsp, &res)) {
                   err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,rsp);
                   return STAT_ADR;}
              set_reg_val(sim->r,4,rsp+8);
              set_reg_val(sim->r,regA,res);
              sim->pc=next_pc;
              break;
        }
