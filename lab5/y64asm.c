...
parse_t parse_instr(char **ptr, instr_t **inst)
{
    ...
    instr_t* ins = find_instr(tmp);
    if(ins == NULL) return PARSE_ERR;

    tmp += ins->len;
    if(!IS_END(tmp) && !IS_BLANK(tmp)) return PARSE_ERR;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    char *tmp = *ptr;
    SKIP_BLANK(tmp);
    if(!IS_REG(tmp)) return PARSE_ERR;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    SKIP_BLANK(tmp);
    if(!IS_LETTER(tmp)) return PARSE_ERR;
     ...
     * copy just-skip-blank tmp, or I'll get wrong symbol name
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    char *tmp = *ptr;
    SKIP_BLANK(tmp);
    if(!IS_DIGIT(tmp)) return PARSE_ERR;

    /* calculate the digit, (NOTE: see strtoll()) */
    long val = (long)strtoull(tmp, &tmp, 0);
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    if(IS_IMM(tmp)){
        ...
        if(parse_digit(&tmp, value) == PARSE_DIGIT){
            *ptr = tmp;
            return PARSE_DIGIT;
        }
        tmp--;
    ...
    if(IS_LETTER(tmp)){
        if(parse_symbol(&tmp, name) == PARSE_SYMBOL){
            *ptr = tmp;
            return PARSE_SYMBOL;
        }
    }
...
parse_t parse_mem(char **ptr, long *value, regid_t *regid)
{
    ...
    *value    = 0;
    ...
    if(*tmp != '(') return PARSE_ERR;

    // skip '('
    tmp++;

    if(parse_reg(&tmp, regid) == PARSE_ERR) return PARSE_ERR;
